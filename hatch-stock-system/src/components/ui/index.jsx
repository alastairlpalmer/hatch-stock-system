...
export function Button({ 
  ...
  const baseStyles = 'inline-flex items-center justify-center font-medium rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900';

  const variants = {
    primary: 'bg-hatch-green text-hatch-cream hover:bg-emerald-500 focus:ring-emerald-500',
    secondary: 'bg-zinc-700 text-zinc-200 hover:bg-zinc-600 focus:ring-zinc-500',
    ghost: 'text-zinc-300 hover:text-hatch-cream hover:bg-zinc-800/60',
...
export function Input({ 
  ...
  return (
    <div className={className}>
      ...
      <input
        className={`w-full bg-zinc-900/60 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 ${
          error ? 'border-red-500' : 'border-zinc-700/60'
        }`}
        {...props}
      />
      {error && (
...
export function Select({ 
  ...
  return (
    <div className={className}>
      ...
      <select
        className="w-full bg-zinc-900/60 border border-zinc-700/60 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500"
        {...props}
      >
        {options.map((opt) => (
