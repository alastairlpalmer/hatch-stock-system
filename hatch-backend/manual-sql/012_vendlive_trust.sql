...
--    and an unmapped auto-created row (sales namespace), move the sales id
--    onto the mapped row and delete the duplicate.
CREATE TEMP TABLE _vmm_dups AS
...
WHERE id IN (SELECT dup_id FROM _vmm_dups);

-- The duplicate's vendlive_machine_id WAS the sales-feed id (webhook ingest
-- stored it there): carry it to the surviving row's sales_machine_id.
