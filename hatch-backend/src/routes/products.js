...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const product = await prisma.product.create({
    data: {
      ...
      // != null, not truthiness: a genuine £0 cost/price was silently
      // discarded as null before.
      unitCost: unitCost != null && unitCost !== '' ? parseFloat(unitCost) : null,
      salePrice: salePrice != null && salePrice !== '' ? parseFloat(salePrice) : null,
      unitsPerBox: parseInt(unitsPerBox) > 0 ? parseInt(unitsPerBox) : 1,
...
router.put('/:sku', asyncHandler(async (req, res) => {
  ...
  const product = await prisma.product.update({
    ...
    data: {
      ...
      ...(unitCost !== undefined && { unitCost: unitCost != null && unitCost !== '' ? parseFloat(unitCost) : null }),
      ...(salePrice !== undefined && { salePrice: salePrice != null && salePrice !== '' ? parseFloat(salePrice) : null }),
      ...(unitsPerBox !== undefined && { unitsPerBox: parseInt(unitsPerBox) > 0 ? parseInt(unitsPerBox) : 1 }),
